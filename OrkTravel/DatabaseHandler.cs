...
namespace OrkTravel
{
    internal class DatabaseHandler
    {
        ...
        public LoginDetails Login(string username, string password)
        {
            using (conn = new MySqlConnection(connectionString)) 
            {
                try
                {
                    ...
                    MySqlCommand cmd = new MySqlCommand("SELECT traveller_id FROM ot_traveller WHERE traveller_email = @username AND BINARY traveller_password = @password;", conn);
        ...
        public List<string> FetchBookings(string email)
        {
            List<string> result = new List<string>();
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    string query = @"SELECT ot_traveller_booking.tb_booking_id
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                // Fetch first column value as string
        ...
        public List<string> FetchCancelTourDetails(string bookingID)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    string query = @"SELECT ot_tour.tour_destination, 
                                    ot_booking.booking_people, 
                                    ot_booking.booking_departure, 
                                    ot_booking.booking_return
                             FROM ot_tour
                             JOIN ot_booking 
                             ON ot_booking.booking_tour_id = ot_tour.tour_code
                             WHERE ot_booking.booking_id = @BookingID;";
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                tourDetails.Add(reader.GetString(0)); // tour_destination
                                tourDetails.Add(reader.GetInt32(1).ToString()); // booking_people
                                tourDetails.Add(reader.GetDateTime(2).ToString("yyyy-MM-dd")); // booking_departure
                                tourDetails.Add(reader.GetDateTime(3).ToString("yyyy-MM-dd")); // booking_return
                            }
                        }
                    }
                }
            ...
            {
                Console.WriteLine($"An error occurred while fetching tour details: {ex.Message}");
            }
        ...
        public bool CancelBooking(string bookingID)
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    string query = @"CALL `ot_delete_booking` (@BookingID);";
